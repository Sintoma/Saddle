...
\begin{theorem}
	...
	Assume that the kernel $K$ of the operator $L_K$ satisfies
	$$
...
\begin{proof}
	...
	\eeqc
	\end{equation}
	Note that the existence of a weak solution to the previous problem is given by the Riesz representation theorem. Moreover, by standard regularity results (see Section \ref{Subsec:Regularity})\todo{Igual lo quitamos}, $\phi_R$ is in fact a classical solution and by the maximum principle, $\phi_R>0$ in $B_R$.
