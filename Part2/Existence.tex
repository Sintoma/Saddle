...
\begin{proposition}
	...
	\eeqc
	$$
	with $f$ an odd $C^2$ function and $\varphi$ a doubly radial \todo{doubly radial odd function mejor??} function satisfying $\varphi (x) = - \varphi(x^\star)$.
...
\begin{proof}
	...
	$$
	and that each function $u_k$ is doubly radial and odd with respect to $\ccal$. Finally, by a compactness \todo{quizas decir algo mas de esto} argument we see that, up to a subsequence, $u_k$ converges in $C^2$ to the desired solution.
\end{proof}
