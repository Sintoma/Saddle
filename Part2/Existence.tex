...
\begin{proposition}
	...
	Let $L_K\in \lcal_\star(2m,\s)$ for some $0<\s<1$. Assume that $\vsub \leq \vsup$ are two bounded functions being doubly radial and odd with respect to the Simons cone with $\vsub\in C^\s(\R^{2m})$ and satisfying   
	\begin{equation}
	...
	Then, there exists $v\in C^{2s+\varepsilon}(B_R)$ for some $\varepsilon>0$, a solution to
	\begin{equation}
...
\begin{proof}
	...
	\eeqc
	$$
	It is easy to see by induction and the regularity results from Proposition~\ref{Prop:InteriorRegularity} that $v_k\in L^\infty(B_R) \cap C^{2\s+2\epsilon}(B_R)$ for some $\epsilon>0$. Moreover, given $\Omega\subset B_R$ a compact set, then $||v_k||_{C^{2s+2\epsilon}(\Omega)}$ is uniformly bounded in $k$.
	...
	$$
	and that each function $u_k$ is doubly radial and odd with respect to $\ccal$. Finally, by Arzelà-Ascoli theorem and the compact embedding of H\"older spaces we see that, up to a subsequence, $v_k$ converges in $C^{2s+\epsilon}$ to the desired solution.
\end{proof}
