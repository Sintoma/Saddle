...
Note that the expression \eqref{Eq:OperatorOddF} has an integro-differential part plus a zero order term with a positive coefficient. Thus, the natural assumption to make for that operator to have a maximum principle is that its ``kernel'' is positive. That is, $\overline{K}(x, y) - \overline{K}(x, y^\star)>0$. This lead us in \cite{FelipeSanz-Perela:IntegroDifferentialI} to introduce the following definition.
...
One of the main results in \cite{FelipeSanz-Perela:IntegroDifferentialI} established necessary and sufficient conditions on the original kernel $K$ so that $L_K$ belong to $\lcal_\star$. This is given in the next result.
