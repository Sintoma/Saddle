...
\begin{proposition}
	...
	Let $\Omega \subset \ocal$ be an open set (not necessarily bounded) and let $L_K  \in \lcal_\star$. Let $v\in C^\alpha(\Omega)\cap L^\infty(\R^{2m})$, for some $\alpha > 2\gamma$, be a doubly radial function satisfying
	$$
	\beqc{\PDEsystem}
...
\begin{proposition}
	...
	Let $L_K  \in \lcal_\star$ and let $v\in C^\alpha(\Omega)\cap L^\infty(\R^{2m})$, for some $\alpha > 2\gamma$, be a doubly radial function satisfying
	\begin{equation}
	...
	\eeqc
	\end{equation}
	Under these assumptions there exists $\overline{\varepsilon}>0$ depending only on $\lambda, m, \s$ and $||c_-||_{L^\infty}$ such that, if $\varepsilon<\overline{\varepsilon}$, then $v \leq 0$ in $H$. \todo{Pensar si hace falta la hipotesis en infinito}
\end{proposition}
...
\begin{proof}[Proof of Proposition~\ref{Prop:MaximumPrincipleLinearized}]
	...
	We claim that $v - \underline{\tau} u \not \equiv 0$. Indeed, if $v - \underline{\tau} u \equiv 0$ then $v = \underline{\tau} u$ and thus, by using \eqref{Eq:uSupersolLinearized}, the equation for $v$ and the fact that $\underline{\tau} > 0$, we get 
	$$
...
\begin{proof}[Proof of point (ii) of Lemma~\ref{Lemma:SecondDerivativeLayer}]
	...
	$$
	All the arguments are the same, using the maximum principle of Proposition~\ref{Prop:MaxPrpNarrowOdd} in the domain $(0,\varepsilon)$, and yield that $v\leq 0$ in $(0,+\infty)$.
