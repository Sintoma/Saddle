...
\begin{proposition}[\cite{RosOton-Survey,SerraC2s+alphaRegularity}]
	...
	\todo{Añadir boundary regularity o aqui o mas tarde}
\end{proposition}
...
\begin{remark}
	...
	Note that, by standard regularity results for $L_K$, we have that $\phi_1 \in C^{\alpha_0}(\overline{\Omega})\cap C^\infty(\Omega)$ for some $0<\alpha_0<\s$, and the regularity up to the boundary is optimal (\todo[color=red]{REFERENCIA!!}see \cite{RosOton-Survey} and the references therein for the details). Due to this and the fact that $\phi_1 >0$ in $\Omega\cap \ocal$ while $\phi_1=0$ in $\R^{2m}\setminus \Omega$, it is easy to check by using \eqref{Eq:OperatorOddF} that $-\infty <L_K \phi_1 < 0$ in $\ocal\setminus \overline{\Omega}$ and $L_K \phi_1 = -\infty$ in $\partial \Omega \cap \ocal$.
\end{remark}
