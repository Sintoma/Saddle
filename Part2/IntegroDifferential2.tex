...
In the local case, a deep result of Savin \cite{Savin-DeGiorgi} states that minimizers of the Allen-Cahn equation $-\Delta u = f(u)$ in $\R^n$ are one-dimensional if $n\leq 7$. On the other hand, Liu, Wang, and Wei \cite{LiuWangWei} have constructed minimizers in dimensions $n\geq 8$ which are not one-dimensional. Let us describe this last result and the connection with the saddle-shaped solution.
