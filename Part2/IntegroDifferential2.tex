...
\begin{theorem}[Existence and uniqueness of saddle-shaped solution]
...
The existence part of this theorem was already proved in part I \cite{FelipeSanz-Perela:IntegroDifferentialI} using variational techniques. Here, we show that existence can also be proved using, instead, the monotone iteration method. Let us remark that in both methods it is crucial to have the positivity condition \eqref{Eq:KernelInequality}. To establish the uniqueness of saddle-shaped solution we will need two ingredients: the asymptotic behavior of saddle-shaped solutions and a maximum principle for the linearized operator in $\ocal$. Both results will be described later on.
...
The second ingredient to prove the uniqueness of saddle-shaped solutions is the following maximum principle in $\ocal$ for the linearized operator $L_K - f'(u)$.
...
\begin{theorem}
	...
	Let $L_K\in \lcal_0(n,\s,\lambda, \Lambda)$ and let $v$ be a bounded solution to one of these two problems:
...
Let us now make some final comments on a related issue concerning one-dimensional symmetry of minimizers. In the local case, a deep result of Savin \cite{Savin-DeGiorgi} states that minimizers of the Allen-Cahn equation $-\Delta u = f(u)$ in $\R^n$ are one-dimensional if $n\leq 7$. On the other hand, Liu, Wang, and Wei \cite{LiuWangWei} have constructed minimizers in dimensions $n\geq 8$ which are not one-dimensional. We should mention that the same question for stable solutions (instead of minimizers) is still largely open, only solved in dimension $n=2$ (see \cite{GhoussoubGui,BerestyckiCaffarelliNiremberg-Qualitative}).


Let us describe briefly the result of Liu, Wang, and Wei \cite{LiuWangWei} and its connection with the saddle-shaped solution. Recall first that in $\R^8$ there exists a foliation $\Gamma_\lambda^\pm$ of minimal surfaces such that $\Gamma_0$ is the Simons cone and $\Gamma_\lambda^+$ and $\Gamma_\lambda^-$, with $\lambda >0$, are asymptotic to $\ccal$ and lie at each side of the cone respectively. This foliation is crucial in the proof of the minimality of the Simons cone in $\R^8$, see \cite{CabrePoggesi} and the references therein. In \cite{LiuWangWei}, Liu, Wang, and Wei prove that there exists an ordered family of minimizers $W_\lambda$ that are not one-dimensional and such that their zero level set is asymptotic to the cone $\ccal$. This family of solutions is parametrized by the same parameter as $\Gamma_\lambda^\pm$, which is related to how close to the Simons cone is the level set $\{W_\lambda=0\}$. Indeed, the results in \cite{LiuWangWei} suggest that for $\lambda=0$, the zero level set of $W_0$ should be the Simons cone, and thus it is conjectured that  $W_0$ is precisely the saddle-shaped solution. However, the construction in \cite{LiuWangWei} is only possible for $\lambda$ large, and therefore the previous conjecture is an open problem.

Concerning the fractional problem, Savin \cite{Savin-Fractional, Savin-Fractional2} extended his results for the Laplacian to the powers $\s \in [1/2,1)$, by proving that minimizers of the equation $\fraclaplacian u = f(u)$ in $\R^n$ are one-dimensional if $n\leq 7$. In the case $\s \in (0,1/2)$, Dipierro, Serra, and Valdinoci \cite{DipierroSerraValdinoci} proved that minimizers are one-dimensional provided that their level sets are asymptotically flat. Therefore, if one could prove a classification result for nonlocal minimal cones in some dimension $n$, this would entail the one-dimensional symmetry of minimizers to $\fraclaplacian u = f(u)$ in $\R^{n-1}$. As mentioned above, the classification of nonlocal minimal cones is still a fundamental open problem in dimensions $n\geq 3$. The one-dimensional symmetry of stable solutions is also largely open, only solved in dimension $n=2$ (see \cite{CabreSolaMorales, CabreSireII}). 
...
With these ingredients, we can proceed with the proof of the existence part in Theorem~\ref{Th:ExistenceUniqueness}.

\begin{proof}[Proof of Theorem~\ref{Th:ExistenceUniqueness}]
...
\begin{proof}[Proof of uniqueness in Theorem~\ref{Th:ExistenceUniqueness}]
