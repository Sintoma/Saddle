...
\begin{proposition}
	...
	Let $L_K $ be a symmetric and translation invariant integro-differential operator of the form \eqref{Eq:DefOfLu} with kernel $K:\R^n\setminus \{0\} \to (0,+\infty) $ satisfying \eqref{Eq:Symmetry&IntegrabilityOfK}. Define the one dimensional kernel $K_1 : \R \setminus \{0\} \to (0,+\infty) $ by
	\begin{equation}
	...
	\begin{enumerate}[label=(\roman{*})]
		...
		\item If $L_K \in \mathcal{L}_0 (n,\s,\lambda,\Lambda)$, then   $L_{K_1} \in \mathcal{L}_0 (1,\s,\lambda,\Lambda)$. Moreover, if $L_K $ is the fractional Laplacian in dimension $n$, then $L_{K_1}$ is the fractional Laplacian in dimension $1$.
