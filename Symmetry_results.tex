...
This section is devoted to prove the following two symmetry results. Both results will be needed in the following section to establish the asymptotic behavior of the saddle-shaped solution. The first one is a result for positive solutions in the whole space.

\begin{theorem}
	\label{Th:SymmetryWholeSpace}
	Let $L\in \lcal_0$ and let $v$ be a bounded solution to
	\begin{equation}
	\label{Eq:PositiveWholeSpace}
	\beqc{\PDEsystem}
	L v &=& f(v) & \textrm{ in }\R^n\,,\\
...
\begin{theorem}
	...
	Let $L\in \lcal_0$ and let $v$ be a bounded solution to one of these two problems
...
The result in the case of problem \eqref{Eq:P2} will not be used in this paper. However, since the proof is analogous as the one for \eqref{Eq:P1}, we include it here for completeness and further reference.
...
Let  $v \in C^{2\s+\epsilon}(B_R\times(0,T])$ be bounded by above and let $L$ be an integro-differential operator of the form \eqref{Eq:DefOfLu} 
...
There is no bounded solution to $Lv=1$ in $\R^n$ for any $L\in \lcal_0$.
\end{lemma}
...
\eeqc
\end{equation*}
By the Liouville Theorem for the operator $L$ (it is proved exactly as in \cite{RosOtonSerra-Stable}, see also \cite{SerraC2s+alphaRegularity}), $v_{x_i}$ is constant. Hence, $\nabla v$ is constant, and thus $v$ is affine. But since $u$ is bounded, $v$ must be constant too, and we arrive to a contradiction with $Lv=1$.
\end{proof}

\begin{lemma}
\label{Lemma:SolBall}
Let $L\in \lcal_0$ and let $R>0$ be given. Then, there exists $\phi_R$ a solution of
\begin{equation*}
\beqc{\PDEsystem}
...
\begin{theorem}
\label{Th:ParaMaxPrp}
Let $L\in \lcal_0$ and let $v$ be a bounded function such that
\begin{equation*}
\beqc{\PDEsystem}
...
\begin{equation*}
\beqc{\PDEsystem}
\partial_t \tilde{v} + L \tilde{v} &\leq& 0 & \textrm{ in } \Omega \subset\R^n\times(0,+\infty)\,,\\
...
With the parabolic maximum principle we can now establish Theorem~\ref{Th:SymmetryWholeSpace}. The proof follows the ideas of Berestycki, Hamel and Nadirashvili from Theorem 2.2. in \cite{BerestyckiHamelNadi} but adapted to the whole space and with a nonlocal operator.
...
By contradiction, we will assume $m=0$. Then, there exists a sequence $\{x_k\}$ such that $v(x_k)\rightarrow 0$ as $k \rightarrow +\infty$. By the Harnack Inequality from Di Casto, Kuusi and Palatucci in \cite{DiCastoKuusiPalatucci}, given any $R>0$ we have
\todo[inline, color=red]{MIRAR LO DE MATTEO!!!!}
\begin{equation}
\label{Eq:Harnack}
\sup_{B_R(x_k)}v \leq C_R \inf_{B_R(x_k)}v \leq C \, v(x_k) \rightarrow 0 \,\,\text{as}\,\, k\rightarrow +\infty.
\end{equation}
...
L v - \frac{f'(0)}{2}v \geq 0 \,\,\textrm{ in }\ B_R(x_{M(R)})\,. 
\end{align}
...
\begin{align*}
0 &\leq \int_{\R^n} Lv\,\frac{w^2}{v} \d x - \frac{f'(0)}{2}\int_{\R^n} w^2 \d x \\
...
\begin{theorem}
	...
	$$
	where $C$ is a constant depending on \todo{Poner, depende de la weak harnack}.
\end{theorem}
...
\begin{corollary}
	...
	\eeqc
	$$
	with $c(x)$ bounded below and $L\in \lcal_0(n,\s)$.
...
\begin{proposition}
	...
	Let $H$ be a half-space in $\R^n$, and denote by $x^*$ the reflection of any point $x$ with respect to the hyperplane $\partial H$. Let $L\in \lcal_0(n,\s)$ with a positive kernel $K$ satisfying
	\begin{equation}
	...
	\begin{equation*}
	\beqc{\PDEsystem}
	L v &\geq& c(x)\,v  &\textrm{ in } \Omega\subset H,\\
...
\begin{proposition}
	...
	Let $v$ be a bounded solution of one of the problems \eqref{Eq:P1} or \eqref{Eq:P2}, with $L\in \lcal_0(n,\s)$ with a positive kernel $K$ satisfying \eqref{Eq:KernelSymmetry} and $f$ a Lipschitz nonlinearity such that $f>0$ in $(0,||v||_{L^\infty(\R^n_+)})$. Then,
	$$ 
...
\begin{proof}
	...
	$$
	\beqc{\PDEsystem} 
	L w_\lambda &=& c_\lambda(x)\,w_\lambda  &\textrm{ in } \Sigma_\lambda\subset H_\lambda,\\ 
...
Let $L\in \lcal_0(n,\s)$ and let $v$ be a bounded solution of one of the following problems
\begin{equation}
\leqnomode
\tag{P3}
\label{Eq:P3}
