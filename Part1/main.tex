...
\usepackage{newclude}
%Use include*{} for not jumps between sections
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
This is the first of two papers concerning saddle-shaped solutions to the semilinear equation $L_K u = f(u)$ in $\R^{2m}$. Here $L_K$ is a linear integro-differential operator which is uniformly elliptic, and $f$ is of Allen-Cahn type. 

Saddle-shaped solutions are doubly radial, odd with respect to the Simons cone $\{(x', x'') \in \R^m \times \R^m \, : \, |x'| = |x''|\}$, and vanish only on this set. 
%By the odd symmetry, one must think that the operator $L_K$ acts on functions $u$ defined only on one side of the Simons cone, $\{|x'|>|x''|\}$, and that vanish on it. After reflecting them oddly, one can then compute $L_K u$. This makes that the operator has indeed a kernel on $\{|x'|>|x''|\}$ which is different from $K$. The positivity of the new kernel is of crucial importance in order to be in front of an elliptic problem. 
By the odd symmetry, $L_K$ coincides with a new operator $L_K^\ocal$ which acts on functions defined only on one side of the Simons cone, $\{|x'|>|x''|\}$, and that vanish on it. This operator $L_K^\ocal$, which corresponds to reflect a function oddly and then apply $L_K$, has indeed a kernel on $\{|x'|>|x''|\}$ which is different from $K$. The positivity of the new kernel is of crucial importance in order to be in front of an elliptic problem. 



In this first paper, part~I, we characterize the kernels $K$ for which the new kernel is positive and therefore one can develop a theory on the saddle-shaped solution for $L_K u=f(u)$. The condition turns out to be that $K$ is radially symmetric and $\tau\to K(\sqrt \tau)$ is a strictly convex function. Under the previous assumptions, we prove an energy estimate for doubly radial odd minimizers and the existence of saddle-shaped solutions.
...
\include{OperatorOddF}

\include{EnergyForOddF}

\include{EnergyEstimate}
