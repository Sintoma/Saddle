...
Therefore, assume now that $u$ solves $L_K u = f(u)$ in $B_R$ and that $|u|\leq 1$ in $\R^n$ with
...
Since $L_K w$ is invariant by $O(m)^2$ and $w(R \cdot) = w(\cdot)$ for every $R\in O(m)^2$,
\begin{align*}
L_K w(x) &= \average_{O(m)^2} Lw(Rx)\d R =  \average_{O(m)^2} \int_{\R^{2m}} \{w(x) - w(y)\}K(|Rx - y|) \d y \d R\\
