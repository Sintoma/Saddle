...
\begin{proof}
	...
	$$
	Note that we can add and subtract $2\,w(x)\overline{K}(x, y^\star)$ in the last integral since it is integrable with respect to $y$ in $\ocal$. This is a consequence of \eqref{Eq:ZeroOrderTerm}. 
	
	Let us show now \eqref{Eq:ZeroOrderTerm}. The upper bound is the simplest one since we only need to use the ellipticity of the kernel and the inclusion $\ical \subset \{y\in\R^{2m}:|x-y|\geq \dist(x,\ccal)\}$. That is,
	\begin{align*}
...
&= C \dist(x,\ccal)^{-2s}.
	\end{align*}
	For the lower bound, let us define $x_0 = x/\dist(x,\ccal)$. Then,
	\begin{align*}
	...
	with a constant independent of $x_0$. To establish this claim we will use three facts. The first one is that, since $\dist(x_0,\ccal) = 1$, then $B_{3k+3}(\overline{x_0}) \setminus B_{3k+2}(\overline{x_0}) \subset  B_{3k+4}(x_0)\setminus B_{3k+1}(x_0)$ for every $k\geq 0$, where $\overline{x_0}$ is the projection of $x_0$ into the cone $\ccal$. The second one is the identity $\ical = \ical \cap B_1^c(x_0)$, that also follows from being $\dist(x_0,\ccal) = 1$. Finally, we use a property of the Simons cone: $|B_R(z) \cap \ical| = 1/2 |B_R|$ if $z\in \ccal$ (see Lemma 2.5 in \cite{Felipe-Sanz-Perela:SaddleFractional} for the proof). Therefore,
	\begin{align*}
...
%let $x_0 = (|x'|e, |x''|e)$ and $y_0 = (|y'|e, |y''|e)$, for an arbitrary unitary vector $e \in \Sph^{m-1} \subset \R^m$. Hence, since $\overline{K}$ is invariant by $O(m)^2$,
%$$
...
\label{Prop:WeakMaximumPrincipleForOddFunctions} Let $\Omega \subset \ocal$ and let $L_K  \in \lcal_\star (2m,  \s)$.  Let $u\in C^{\alpha}_{\loc}(\Omega)\cap L^\infty(\R^{2m})$ with $\alpha > 2\s$ be a doubly radial function which is odd with respect to the Simons cone. Assume that
$$
\beqc{\PDEsystem}
