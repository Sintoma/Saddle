...
\begin{proof}
	...
	\end{align*}
	\todo[inline]{Mejorar lo de arriba, más simple.}
\end{proof}
...
\begin{remark}
Note that since the operator $L_K$ includes itself a positive zero order term in addition to the integro-differential part, the condition $c\geq 0$ in point $(i)$ of the previous proposition can be lightly relaxed. Indeed, if we follow the proof of the result, we can deduce that the hypothesis on $c$ that we can assume is 
