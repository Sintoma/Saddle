...
$$
where $\overline{K}$, defined by
\begin{equation*}
%\label{Eq:KbarDef}
\overline{K}(x,y) := \average_{O(m)^2} K(|Rx - y|)\d R\,,
\end{equation*}
is symmetric and invariant by $O(m)^2$ in both arguments.
...
\begin{remark}
	We do not know... si no es $C^1$ \todo{Acabar}
\end{remark}
