...
The main result, used in the characterization of $\lcal_\star$ in Theorem \todo{Poner teorema en la introducción}, is the following.
...
To prove this result, we need some lemmas on convex functions. The first one is the following.
...
The inequality $g(A) + g(D) \geq g(B) + g(C)$ is trivial by the monotonicity of $g$. Moreover, if $A>B$ or $D>C$ we obtain the strict inequality since $g$ is in fact increasing.
...
$(-1/c_2,1/c_2)$ for all $c_1>0$. Hence, using Corollary~\ref{Cor:gConvex<->K(sqrt)convex} we deduce
...
In this appendix we present an auxiliary computation that is needed in Section~\ref{Sec:Preliminaries} in order to characterize the operators that belong to $\lcal_\star$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The integro-differential operator $L$ in the $(s,t)$ variables}
...
Now we rewrite the kernel inequality \eqref{Eq:KernelInequalityReflexion} in $(s,t)$ variables. We
...
\eqref{Eq:KernelSTVariables2} with $K(\sqrt{\cdot})$ strictly convex. Then, if $s>t>0$ and $\sigma > \tau>0$\todo{Check if we can put
$t=0$ or $\tau = 0$}, we have
\begin{equation*}
