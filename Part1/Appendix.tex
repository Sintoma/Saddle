...
C_{\loc}^\alpha(\R^{2m})$ with $\alpha > 2\s$ be a doubly radial function, i.e., depending only on the variables $s$ and $t$. Let $L_K$ be a rotational invariant operator, $K(y) = K(|y|)$, of the form \eqref{Eq:DefOfLu}. Then, if we define $\tilde{w}:(0,+\infty)\times (0,+\infty) \to \R$ by $\tilde{w}(s,t) = w(s,0,...,0,t,0,...,0)$, it holds
