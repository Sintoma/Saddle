...
\begin{proof}
Note first that if $x\in \Omega_S$, then $d_S(x)=1$ and the result is trivial by the Lipschitz continuity of $\Psi_S$. Hence, we only need to establish the result for the case $x\in B_S\cap \{\dist(\cdot,\ccal)\geq 1\} \cap \ocal$ and $y\in \ocal$. Under these assumptions, we have that $\Psi_S(x)=-1$ (see Figure~\ref{Fig:PsiSandOmegaS}) and $d_S(x) = \min\{S+1-|x|,\dist(x,\ccal)\} \leq S+1-|x|$. Therefore, under the assumption $|x-y|\leq d_S(x)$, it holds
