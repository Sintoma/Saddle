...
To overcome this problem, we will consider a function $w$ defined in $B_{S+2}\cap \ocal$ and satisfying the four previous assumptions on $v$. In addition, we will require that $w$ is doubly radial and that it vanishes on the Simons cone (then we will consider its odd extension through $\ccal$). Apart from this, we will need some technical details, described next. 
...
\end{equation} 
with $\mu>0$ a positive number that will be chosen later. This will be exactly the set where $w\not \equiv -1$ inside $B_{S+2}$. It is easy to see that its measure is of order $2m-1$. That is,
 \begin{equation}
...
In second place, we also need to adapt the function $d(x)$ of \cite{SavinValdinoci-EnergyEstimate} to our setting of odd functions, and we will consider the following one: 
$$ 
d_S(x) := \max\left\{1,\min\{S+1-|x|,\mu \dist(x,\ccal)\} \right\},  
$$
where $\mu$ is the same number as in \eqref{Eq:DefOmegaS}. Note that the set $\Omega_S$ is the preimage of $1$ through $d_S$ in $\overline{B_{S+2}}$ ---see Figure~\ref{Fig:PsiSandOmegaS}~(b). Furthermore, if $x\in B_{S+2}\setminus \Omega_S = B_S \cap \{\mu\dist(\cdot,\ccal) \geq 1\}$, the expression of $d_S(x)$ is simpler:
\begin{align*}
     \restr{d_S}{B_{S+2}\setminus \Omega_S} (x) 
	 = \min \{\dist(x, \partial_{B_{S+1}}),\mu \dist(x,\ccal)\} .
\end{align*}
As we will see in the proof of Theorem~\ref{Th:EnergyEstimate}, by splitting some double integrals in sets where $|x-y|\geq d_S(x)$ and $|x-y|\leq d_S(x)$, we will accomplish the precise bound \eqref{Eq:EnergyEstimate}. 

Last, we mention now a technical detail that we will need when performing our estimates, and that is related with this function $d_S$. We will assume that the competitor $w$ satisfies
$$ |w(x)-w(y)| \leq \frac{C}{\dist(x,\ccal)}|x-y| $$
whenever $x,y \in B_{S+1}\cap \ocal$, $\mu \dist(x,\ccal)\geq 1$ and $\mu \dist(y,\ccal)\leq 1$, and with $C$ a constant independent of $R$ and $S$. This will be used in one estimate in the proof of Theorem~\ref{Th:EnergyEstimate}, when $|x-y|\leq d_S(x)$ 
...
\begin{lemma}
	...
	Let $S>1$ and $R > S + 4$. Then, there exists a function $w:\overline{ B_{S+2}\cap \ocal} \to \R$ satisfying the following:
	
	\begin{enumerate}[label=\textcolor{red}{H\arabic*.}, ref=H\arabic*]
	\item
...
\end{cases}
\end{equation}
where $\mu$ is a positive number that depends on the Lipschitz constant of $u$ in $B_{S+2}$ and makes \cref{Eq:wH3} to be satisfied.
...
\begin{proof}
	By definition, it is clear that $w$ is an admissible competitor. Hence, since $u$ is a minimizer
	$$ \ecal (u, B_R) \leq \ecal (w, B_R). $$
	But since $u\equiv w$ in $B_{S+2}^c$, it follows that
	...
	Therefore we only need to estimate $\ecal (w, B_{S+2})$. First, note that using the ellipticity of the kernel $K$ and the change of variables given by $(\cdot)^\star$ we obtain
	...
	Since $w=\pm 1$ in $B_{S+2} \setminus \omega_S$ and $G(1) = G(-1) = 0$, it is clear that
	$$ \int_{B_{S+2}} G(w) = \int_{\Omega_S} G(w) \leq C |\Omega_S| \leq C S^{2m-1}. $$  
	
	\textbf{Estimate for the kinetic energy}
	Here we split the integral in three terms. That is,
	\begin{align*}
	...
	&\leq C \int_{\Omega_{S}\cap \ocal} \d x \int_0^1 \d r \, r^{1-2\s} + \int_{\Omega_{S}\cap \ocal} \d x \int_1^\infty \d r \, r^{-1-2\s} \\
	&= C |\Omega_S| \leq C \ S^{2m-1}.
	\end{align*}
	In the first addend we have used the Lipschitz condition of $w$ in $\overline{B_{S+3}}$ while in the second one we have only used that it is bounded. 
	\begin{align*}
	I_2 &= \int_{(B_S\setminus \Omega_S)\cap \ocal} \d x \int_{B_{S+1}\cap \ocal} \d y \frac{|w(x)-w(y)|^2}{|x-y|^{2m+2\s}} \leq \int_{(B_S\setminus \Omega_S)\cap \ocal} \d x \int_{\R^{2m}} \d y \frac{|w(x)-w(y)|^2}{|x-y|^{2m+2\s}}  \\
	&= \int_{(B_S\setminus \Omega_S)\cap \ocal} \d x \int_{|x-y|\leq d_S(x)} \d y \frac{|w(x)-w(y)|^2}{|x-y|^{2m+2\s}} + \int_{(B_S\setminus \Omega_S)\cap \ocal} \d x \int_{|x-y|\geq d_S(x)} \d y \frac{|w(x)-w(y)|^2}{|x-y|^{2m+2\s}} \\
	&\leq C \int_{(B_S\setminus \Omega_S)\cap \ocal} \d x d_S(x)^{-2\s}.
	\end{align*}
	Here we have used the fact that $w$ is bounded for the first term, and property (iv) of $w$ for the second one.
	
	In order to estimate $I_3$ we first need to show that if $x\in (B_S\setminus \Omega_S) \cap \ocal$ and $y\in (B_{S+1}\cap \ocal)^c = I \cup B_{S+1}^c$, then $|x-y|\geq C d_S(x)$. That is, on the one hand it is clear that if $y\in B_{S+1}^c$ then $|x-y|\geq \dist(x,\partial B_{S+1})\geq d_S(x)$. On the other hand, if $y\in \ical$ then $|x-y|\geq \dist(x,\ccal) \geq \frac{1}{\mu} d_S(x)$. Thus, we obtain
	\begin{align*}
	...
	$$ \ecal(w,B_{S+2}) \leq C \left( \int_{(B_S\setminus \Omega_S)\cap \ocal} \d x d_S(x)^{-2\s} + S^{2m-1}\right), $$
	which can be easily estimate. That is,
	\begin{align*}
	...
	\end{cases}
	\end{align*}
	Finally we obtained the desired energy estimate by combaning the ones of the kinetic and potential energy.
\end{proof}
