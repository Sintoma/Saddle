...
\end{equation}
and we define it in $\ical$ by considering its odd reflection. It is clear that $\Psi_S$ is a bounded function with $||\Psi_S||_{L^\infty(\R^{2m})}=1$. In our arguments we will also use the following function and set:
...
\begin{proof}
Note first that if $x\in \Omega_S$, then $d_S(x)=1$ and the result is trivial by the Lipschitz continuity of $\Psi_S$. Hence, we only need to establish the result for the case $x\in B_S\cap \{\dist(x,\ccal)\geq 1\} \cap \ocal$ and $y\in \ocal$. Under these assumptions, we have that $\Psi_S(x)=-1$ (see Figure~\ref{Fig:PsiSandOmegaS}) and $d_S(x) = \min\{S+1-|x|,\dist(x,\ccal)\}$. Moreover, since $x\in B_S$ and $\dist(x,\ccal)\geq 1$, we get $d_S(x) \leq S+1-|x|$. Therefore, under the assumption $|x-y|\leq d_S(x)$, it holds
$$ |y|\leq |x-y| + |x| \leq d_S(x)+|x| \leq S+1. $$

Thus, we have to show
\begin{equation}
\label{Eq:TriangularIneqProof1}
	|1+\Psi_S(y)|\leq C\dfrac{|x-y|}{\min\{S+1-|x|,\dist(x,\ccal)\}}
\end{equation}
for $x\in B_S\cap \{\dist(x,\ccal)\geq 1\} \cap \ocal$ and $y \in B_{S+1}\cap \ocal$.

Now we distinguish two cases, either $\dist(y,\ccal)\geq 1$ or $\dist(y,\ccal)\leq 1$. Assume first that $y\in B_{S+1} \cap \{\dist(\cdot,\ccal)\geq 1\}\cap \ocal$. Then, $\Psi_S(y)=-1$ and \eqref{Eq:TriangularIneqProof1} is trivial. Thus, it only remains to show \eqref{Eq:TriangularIneqProof1} in the case $y\in B_{S+1} \cap \{\dist(\cdot,\ccal)\leq 1\}\cap \ocal$. Note that under these assumptions, $\Psi_S(x)=-1$ and $\Psi_S(y)=-\dist(y,\ccal)$. 

We claim that for $x \in \{\dist(\cdot,\ccal)\geq 1\}$ and $y \in \{\dist(\cdot,\ccal)\leq 1\}$ it holds
\begin{equation}
\label{Eq:TriangularIneqProof2}
|1-\dist(y,\ccal)|\leq \dfrac{|x-y|}{\dist(x,\ccal)}
\end{equation}
Assuming this claim to be true, we have that if $x\in B_S\cap \{\dist(x,\ccal)\geq 1\} \cap \ocal$ and $y\in B_{S+1} \cap \{\dist(\cdot,\ccal)\leq 1\}\cap \ocal$, then
...
\end{equation}
Therefore, since $\dist(x,\ccal)\geq 1$, we have
\begin{equation} 
\label{Eq:TriangularCone2}
1-|x-y|-\dist(y,\ccal) \leq 1-\dist(x,\ccal) \leq 0
\end{equation}
Now, multiplying \eqref{Eq:TriangularCone} by $|1-\dist(y,\ccal)|$, and using that $\dist(y,\ccal)\leq 1$ and \eqref{Eq:TriangularCone2} we obtain
\begin{align*}
...
\end{cases} $$
with $C>0$ independent of $S$ and depending only on $m$ and $\s$.
\end{lemma}
...
& \leq C\, \left(\int_1^S z^{-2\s} \d z \right)  \left(  \int_0^S  y^{2m-2} \d y \right) \\
& \leq \begin{cases}
C \ S^{2m-2\s}\ \ \ \ &\textrm{if } \ \ \s\in(0,1/2),\\
C\ \log(S)\,S^{2m-2\s}\ \ \ \ &\textrm{if } \ \ \s=1/2,\\
C \ S^{2m-1}\ \ \ \ &\textrm{if } \ \ \s\in(1/2,1).\\
