...
Note that both $\Psi_S$ and $d_S$ are Lipschitz functions, with Lipschitz norm independent of $S$. Moreover $\Psi_S$ is odd and $d_S$ even with respect to the Simons cone. Regarding the set $\overline{\Omega_S}$ (see Figure~\ref{Fig:PsiSandOmegaS}), we can see it as the preimage of $1$ through $d_S$ inside $\overline{B_{S+2}}$.
