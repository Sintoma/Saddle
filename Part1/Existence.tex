...
In this section we establish the existence of saddle-shaped solutions to the Allen-Cahn equation. Is based on the direct method of the calculus of variations, and it uses all the results appearing in Section~\ref{Sec:Nonlocal_AllenCahn_Energy}.
...
Therefore, we have that $u$ is a solution of $L_K u = f(u)$ in $\R^{2m}$ which is doubly radial. Furthermore, $u$ is odd with respect to the Simons cone $\ccal$, i.e., $u(x) = -u(x^\star)$ for $x\in \R^{2m}$, and $0 \leq u\leq 1$ in $\ocal$.
