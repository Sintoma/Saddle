...
To prove \eqref{Eq:wH1} we need to prove first that the minimizer $u$ satisfies $-1\leq u \leq 1$ and that $0\leq u$ in $\ocal$. To see this we use Lemma~\ref{Lemma:DecreaseEnergy}, which uses crucially the expressions for the energy of doubly radial odd functions developed in the previous section and the key inequality \eqref{Eq:KernelInequality} for the kernel $\overline{K}$. Once this is proved, since $\Psi_S \equiv -1$ in $(B_{S+2}\cap \ocal)\setminus \Omega_S$ ---see Figure~\ref{Fig:PsiSandOmegaS}~(b)---, \eqref{Eq:wH4} holds. The point \eqref{Eq:wH2} follows easily and \eqref{Eq:wH5} requires some computations. For the details, see the proof below.

We postpone the proof of Lemma~\ref{Lemma:ExistenceCompetitor} and, assuming it true for the moment, let us establish the energy estimate of Theorem~\ref{Th:EnergyEstimate}. 
...
\begin{proof}[Proof of Lemma~\ref{Lemma:ExistenceCompetitor}] We divide it into two steps.
	...
	First note that since $u$ is a minimizer of $\ecal$ in $B_R$, by Lemma~\ref{Lemma:DecreaseEnergy} we can assume without loss of generality that $-1 \leq u \leq 1$, $u \geq 0$ in $\ocal$, and $u \leq 0$ in $\ical$. Recall that to prove Lemma~\ref{Lemma:DecreaseEnergy} we used crucially the expression of the kinetic energy given by \eqref{Eq:ShortExpressionEnergy} and the ellipticity assumption \eqref{Eq:KernelInequality}.
