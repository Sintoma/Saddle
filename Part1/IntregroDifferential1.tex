...
\begin{proposition}
	...
	As a consequence, if $u\in \widetilde{\H}^K_{0}(\Omega)$ is a doubly minimizer of the energy $\ecal(u,\Omega)$, then it is a weak solution to \eqref{Eq:DirichletProblemOmega}.
\end{proposition}
...
\begin{proof}
	...
	We next show that if $u$ is a minimizer, then it is a weak solution to \eqref{Eq:DirichletProblemOmega}. To see this, we consider perturbations $u +  \varepsilon \xi$ with $\varepsilon>0$. On the one hand, take $\xi \in \widetilde{\H}^K_{0, \,\mathrm{odd}}(B_R)$. Then, since $u$ is a minimizer among functions in $\widetilde{\H}^K_{0, \,\mathrm{odd}}(B_R)$, we get
	$$
	...
	Therefore, 
	$$
	\langle u,\xi \rangle_{\widetilde{\H}^K_0(B_R)} = \langle f(u_R),\xi \rangle_{L^2(B_R)}
	$$
	...
	$$
	for every $\xi \in C^\infty_c(\Omega)$ that is doubly radial, and by the previous reasoning, $u$ is a weak solution to \eqref{Eq:DirichletProblemOmega}.
\end{proof}
...
In our case, the strategy will be the same but adapting some ingredients, namely, the competitor $v$. First, note that the previous construction for $v$ cannot be used in our setting, since it would not produce a doubly radial function $v$ which is odd with respect to the Simons cone $\ccal$. To overcome this problem, we will consider a function $w$ defined in $B_{S+2}\cap \ocal$ and satisfying the four previous assumptions on $v$. In addition, we will require $w$ to be doubly radial and to vanish on the Simons cone (then we will consider its odd extension through $\ccal$).

Let us introduce two definitions. First, let $\mu$ be the Lipschitz constant of $u$ in $\overline{B_{S+3}}$, that is,
\begin{equation} 
\label{Eq:ChoiceMu} 
	\mu = \seminorm{u}_{\mathrm{Lip}(\overline{B_{S+3}})}, 
\end{equation} 
By Proposition~\ref{Prop:WeakSolutionForAllTestFunctions}, we know that $u$ solves $L_K u = f(u)$ in $B_R$ with $R> S+4$. Therefore, by applying repeatedly the estimates \eqref{Eq:C2sEstimateBalls} and \eqref{Eq:Calpha->Calpha+2sEstimateBalls} in balls centered at points of $B_{S+3}$, it is easy to see that
$$
...
\begin{proof}[Proof of Theorem~\ref{Th:EnergyEstimate}]
	...
	$$
	Therefore, to obtain our result it remains to estimate $\ecal (w, B_{S+2})$. 
...
\begin{proof}[Proof of Theorem~\ref{Th:Existence}]
	...
	Now, by Proposition~\ref{Prop:WeakSolutionForAllTestFunctions} and the regularity for operators in $L_K$ (see Remark~\ref{Remark:InteriorRegularity}), we have that $u_R$ is a classical solution to
	$$
	 \beqc{\PDEsystem}
...
\begin{proof}
	...
	%$$
	%with $J$ as in \eqref{Eq:KernelSTVariables1}. Indeed, in $\{y_2 > y_1 > 0\}$ we have that
