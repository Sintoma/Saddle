...
\begin{theorem}
	...
	Let $K$ be a radially symmetric kernel satisfying the convexity assumption \eqref{Eq:SqrtConvex} and such that $L_K\in \lcal_0(2m, \s, \lambda, \Lambda)$. Assume that $G$ is a potential satisfying \eqref{Eq:HipothesesG}. Let $S\geq2$ and let $u\in \widetilde{\H}^K_{0, \mathrm{odd}}(B_R)$ be a doubly radial odd minimizer of $\ecal$ in $B_R$, with $R>S+4$. 
...
\begin{remark}
	...
	\end{equation} 
	for $\alpha \in (0,1)$ and with $C$ depending only on $n$, $\s$, $\lambda$, and $\Lambda$.
